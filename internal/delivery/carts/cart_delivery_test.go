...
func TestGetCart(t *testing.T) {
	...
	delivery := NewCartDelivery(cartUsecase, logger.Sugar())
...
func TestCreateCart(t *testing.T) {
	...
	cartUsecase.EXPECT().CreateCart(ctx, testUserId).Return(uuid.Nil, err)
	delivery.CreateCart(c)
	require.Equal(t, 500, w.Code)
	...
	cartUsecase.EXPECT().CreateCart(ctx, testUserId).Return(testCartId, nil)
	delivery.CreateCart(c)
