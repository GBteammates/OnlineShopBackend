...
func NewOrderDelivery(orderUsecase usecase.IOrderUsecase, logger *zap.SugaredLogger) *OrderDelivery {
...
func (delivery *OrderDelivery) CreateOrder(c *gin.Context) {
	...
	err = delivery.orderUsecase.DeleteCart(ctx, cartModel.Id)
	if err != nil {
	...
	newCartId, err := delivery.orderUsecase.CreateCart(ctx, user.ID)
	if err != nil {
